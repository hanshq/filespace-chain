...
read -r answer
if [ ! "$answer" != "${answer#[Nn]}" ] ;then
  filespace-chaind keys add pools --keyring-backend test
  filespace-chaind keys add user --keyring-backend test

  # Add genesis accounts and provide coins to the accounts
  filespace-chaind genesis add-genesis-account $(filespace-chaind keys show pools --keyring-backend test -a) 1000000000000000000000000uspace
  # Give some uatom to the local-user as well
  filespace-chaind genesis add-genesis-account $(filespace-chaind keys show user --keyring-backend test -a) 1000000000000000000000uspace
fi
