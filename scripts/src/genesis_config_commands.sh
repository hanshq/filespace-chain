...
set_gov_params() {
    ...
    jq '.app_state.gov.deposit_params.min_deposit[0].denom = "uspace"' "$GENESIS_FILE" > "$tmp" && mv "$tmp" "$GENESIS_FILE"
...
set_hub_params() {
    ...
    sed -i'' -e 's/bond_denom": ".*"/bond_denom": "uspace"/' "$GENESIS_FILE"
    sed -i'' -e 's/mint_denom": ".*"/mint_denom": "uspace"/' "$GENESIS_FILE"
...
set_misc_params() {
  jq '.app_state.crisis.constant_fee.denom = "uspace"' "$GENESIS_FILE" > "$tmp" && mv "$tmp" "$GENESIS_FILE"
  jq -r '.app_state.gamm.params.pool_creation_fee[0].denom = "uspace"' "$GENESIS_FILE" > "$tmp" && mv "$tmp" "$GENESIS_FILE"
  jq '.app_state["txfees"]["basedenom"] = "uspace"' "$GENESIS_FILE" > "$tmp" && mv "$tmp" "$GENESIS_FILE"
...
set_bank_denom_metadata() {
  jq '.app_state.bank.denom_metadata = [
  {
    ...
    "description": "Denom metadata for space (uspace)",
    "display": "space",
    "name": "space",
...
enable_monitoring() {
  sed -i'' -e "/telemetrytelemetry/,+8 s/enabled = ./enabled = true/" "$APP_CONFIG_FILE"
